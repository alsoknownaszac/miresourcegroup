...
export const serviceDetailed = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      options: {
        list: [
          ...
          { title: 'Monitor (ICT/Technology)', value: 'Monitor' },
        ],
        layout: 'dropdown',
      },
      validation: (Rule) => Rule.required(),
    ...
    defineField({
      ...
      title: 'Products',
      type: 'array',
      description: 'Product categories offered (e.g. "Desktops", "Laptops"). Leave empty for services without a products section.',
      of: [{ type: 'string' }],
    }),
    defineField({
      name: 'brands',
      title: 'Brands',
      type: 'array',
      description: 'Brand names displayed as logos (e.g. "HP", "DELL"). Leave empty for services without a brands section.',
      of: [{ type: 'string' }],
    }),
    defineField({
      ...
      type: 'text',
      description: 'Optional closing call-to-action paragraph displayed at the bottom of the service panel.',
      rows: 3,
    }),
    defineField({
      name: 'order',
